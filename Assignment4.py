...
# 
# This assignment requires that you to find **at least** two datasets on the web which are related, and that you visualize these datasets to answer a question with the broad topic of **sports or athletics** (see below) for the region of **Kolkata, West Bengal, India**, or **India** more broadly.
# 
# You can merge these datasets with data from different regions if you like! For instance, you might want to compare **Kolkata, West Bengal, India** to Ann Arbor, USA. In that case at least one source file must be about **Kolkata, West Bengal, India**.
# 
...
# 
#  * State the region and the domain category that your data sets are about (e.g., **Kolkata, West Bengal, India** and **sports or athletics**).
...
temp_min1 = df_temp_min.groupby('Month-Day')['Data_Value'].agg({'temp_min_mean_Kolkata': np.mean})
temp_max1 = df_temp_max.groupby('Month-Day')['Data_Value'].agg({'temp_max_mean_Kolkata': np.mean})
...
ax2 = ax1.twinx()
ax1.plot(temp_min1['temp_min_mean_Kolkata']/10, 'y', alpha = 0.75, label = 'Average Low Temperature in Kolkata')
ax1.plot(temp_max1['temp_max_mean_Kolkata']/10, 'r', alpha = 0.5, label = 'Average High Temperature in Kolkata')
